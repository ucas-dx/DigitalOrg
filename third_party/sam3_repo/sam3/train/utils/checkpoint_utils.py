...
def load_state_dict_into_model(
    ...
    # 兼容结构变化：对形状不匹配的参数做扩展或丢弃（例如 num_queries 从 200 -> 800）
    ...
    for k, v in state_dict.items():
        ...
        if v.shape != tgt.shape:
            # 针对 decoder 的 query_embed / reference_points 做兼容扩展
            if (
                ...
                state_dict[k] = new_w
                logging.info(
                    f"Expanded resume checkpoint query_embed from {tuple(v.shape)} to {tuple(tgt.shape)} for key {k}"
                )
            elif (
                ...
                state_dict[k] = new_w
                logging.info(
                    f"Expanded resume checkpoint reference_points from {tuple(v.shape)} to {tuple(tgt.shape)} for key {k}"
                )
            else:
                keys_to_delete.append(k)
                logging.warning(
                    f"Dropping mismatched key from resume checkpoint: {k}, "
                    f"ckpt shape={tuple(v.shape)}, model shape={tuple(tgt.shape)}"
                )
