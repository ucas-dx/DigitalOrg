...
def _load_checkpoint(model, checkpoint_path):
    ...
    # 兼容 num_queries 等结构变化：对形状不匹配的权重做特殊处理或丢弃
    ...
    for k, v in sam3_image_ckpt.items():
        ...
        if v.shape != tgt.shape:
            # 对 decoder query_embed / reference_points 做兼容扩展（例如 200 -> 800）
            if (
                "transformer.decoder.query_embed.weight" in k
                and v.dim() == 2
                and tgt.dim() == 2
                and v.shape[1] == tgt.shape[1]
                and v.shape[0] <= tgt.shape[0]
            ):
                # 复制已有权重到前 v.shape[0] 行，剩余保持初始化
                new_w = tgt.clone()
                new_w[: v.shape[0]] = v
                sam3_image_ckpt[k] = new_w
                print(
                    f"[INFO] Expanded checkpoint query_embed from {tuple(v.shape)} to {tuple(tgt.shape)} for key {k}"
                )
            elif (
                "transformer.decoder.reference_points.weight" in k
                and v.dim() == 2
                and tgt.dim() == 2
                and v.shape[1] == tgt.shape[1]
                and v.shape[0] <= tgt.shape[0]
                ...
                sam3_image_ckpt[k] = new_w
                print(
                    f"[INFO] Expanded checkpoint reference_points from {tuple(v.shape)} to {tuple(tgt.shape)} for key {k}"
                )
            else:
                # 其它形状不匹配的参数直接丢弃，保持当前模型初始化
                keys_to_delete.append(k)
                print(
                    f"[WARN] Drop mismatched key {k}: ckpt shape={tuple(v.shape)}, model shape={tuple(tgt.shape)}"
                )
    for k in keys_to_delete:
        sam3_image_ckpt.pop(k, None)
