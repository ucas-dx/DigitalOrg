...
class SegmentationValidator(DetectionValidator):
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        ...
        # NOTE: 为了适配自定义 ConvNeXt segmentation 头，并确保验证阶段
        # 预测 mask 与 GT mask 在相同分辨率上计算 IoU，这里强制使用
        # process_mask_native（会把 mask 上采样到输入图像尺寸）。
        # 否则，默认的 process_mask（下采样到 1/4）在某些自定义 backbone
        # 配置下可能导致形状不一致的 matmul 错误。
    ...
    def _process_batch(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> dict[str, np.ndarray]:
        ...
        if gt_cls.shape[0] == 0 or preds["cls"].shape[0] == 0:
            ...
            # 在某些自定义 backbone / 头部配置下，GT mask 与预测 mask 的空间分辨率可能不一致，
            # 直接展平后做 matmul 会报维度不匹配错误：
            # RuntimeError: mat1 and mat2 shapes cannot be multiplied (...).
            # 为了健壮性，这里在计算 IoU 前将 GT mask 重采样到预测 mask 的分辨率。
            gt_masks = batch["masks"]
            pred_masks = preds["masks"]

            if gt_masks.ndim != 3 or pred_masks.ndim != 3:
                raise RuntimeError(
                    f"Unexpected mask dims. GT: {gt_masks.shape}, Pred: {pred_masks.shape} (expect N,H,W)."
                )

            if gt_masks.shape[1:] != pred_masks.shape[1:]:
                # 将 GT mask 重采样到预测 mask 的空间分辨率，保持二值属性
                target_h, target_w = pred_masks.shape[1:]
                gt_masks = (
                    F.interpolate(gt_masks[None].float(), size=(target_h, target_w), mode="nearest")[0] > 0.5
                )

            iou = mask_iou(gt_masks.flatten(1).float(), pred_masks.flatten(1).float())  # float, uint8
