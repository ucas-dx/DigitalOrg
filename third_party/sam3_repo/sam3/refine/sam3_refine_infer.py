...
def build_sam3_inst_model(
    ...
    """
    Build SAM3 image model with instance interactivity enabled, as in
    examples/sam3_for_sam1_task_example.ipynb, and wrap it with Sam3Processor.
    """
...
def infer_one_sam3_inst(
    ...
    Returns a list of uint8 masks (H,W) in the model's output resolution.
    """
    if len(boxes) == 0:
        return []
    ...
    # 禁用 autocast，强制在 float32 上跑，以避免 dtype 冲突
...
def infer_one_sam3_with_refine(
    ...
    # ensure model + processor on a single device
    model.to(device_t)
    processor.device = device_t

    # coarse SAM3 masks (and keep tensor version on same device)
    if torch.cuda.is_available():
        autocast_ctx = torch.cuda.amp.autocast
    else:
        class _DummyCtx:
    ...
    # build refine net
    refine_net = ROIRefineNet(variant="atto", convnext_ckpt=None, use_prompt=True, build_prompt=False)
    ...
    # prepare 1024×1024 canvas and boxes
    img_1024 = image.resize((1024, 1024))
    img_np_1024 = np.array(img_1024)
    img_bchw_1024 = torch.from_numpy(img_np_1024).permute(2, 0, 1).unsqueeze(0).float().to(device_t)
    ...
    for i in range(refine_prob_256.shape[0]):
        # paste to full 1024×1024 canvas on same device
