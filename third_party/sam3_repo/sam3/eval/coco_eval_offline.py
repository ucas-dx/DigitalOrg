...
class CocoEvaluatorOfflineWithPredFileEvaluators:
    ...
    def evaluate(self, dumped_file):
        ...
        # 兼容处理：有些 COCO json（如 ExampleDataset/data.json）可能缺少 "info" 字段，
        # 而 pycocotools.Coco.loadRes 默认会假设 self.dataset['info'] 一定存在。
        # 为避免 KeyError，这里在缺失时补一个空的 info。
        if "info" not in self.gt.dataset:
            self.gt.dataset["info"] = {}
        ...
        # Use more detections per image for evaluation (up to 1000 instead of default 100)
        try:
