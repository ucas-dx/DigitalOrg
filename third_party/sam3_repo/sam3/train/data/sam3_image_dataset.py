...
class CustomCocoDetectionAPI(VisionDataset):
    ...
    def _load_images(
        ...
        for current_meta in self.coco.loadImagesFromDatapoint(datapoint_id):
            ...
            path_full = os.path.join(self.root, path)
            try:
                if ".mp4" in path_full and path_full[-4:] == ".mp4":
                    # Going to load a video frame
                    video_path, frame = path_full.split("@")
                    ...
                    with g_pathmgr.open(path_full, "rb") as fopen:
                        all_images.append(
                            (img_id, PILImage.open(fopen).convert("RGB"))
                        )
            except FileNotFoundError:
                # 兼容处理：COCO json 中的 file_name 后缀可能与磁盘实际文件不一致
                # 尝试使用同名不同后缀的文件进行回退搜索
                stem = os.path.splitext(os.path.basename(path_full))[0]
                exts = [".png", ".jpg", ".jpeg", ".tif", ".tiff", ".bmp", ".webp"]
                alt_path = None
                for ext in exts:
                    cand = os.path.join(self.root, f"{stem}{ext}")
                    if g_pathmgr.exists(cand):
                        alt_path = cand
                        break
                if alt_path is None:
                    # 最后再尝试递归搜索一下（代价稍高，但更稳妥）
                    for ext in exts:
                        for root_dir, _, files in os.walk(self.root):
                            for fname in files:
                                if fname.startswith(stem) and fname.lower().endswith(
                                    ext
                                ):
                                    alt_path = os.path.join(root_dir, fname)
                                    break
                ...
                if alt_path is not None:
                    print(
                        f"File not found: {path_full} from dataset: {self.annFile}, "
                        f"fallback to: {alt_path}"
                    )
                    with g_pathmgr.open(alt_path, "rb") as fopen:
