...
class RandomMosaicAPI:
    """
    YOLO-style 4-image mosaic augmentation for single images with bbox.

    Notes:
      - This implementation assumes a Datapoint with a single frame per datapoint (images[0]).
      - It is meant to be used at the dataset level by subclassing Sam3ImageDataset,
        so here we only implement the per-datapoint geometry transform.
      - Bounding boxes are assumed to be in absolute XYXY pixel coordinates on the
        current image size when this transform is applied (i.e. before Normalize).
    """
    ...
    def __init__(
        ...
        min_center: float = 0.3,
        max_center: float = 0.7,
    ):
        self.prob = prob
    ...
    def __call__(self, datapoint: Datapoint, **kwargs):
        # This class only defines the geometry logic for a single image Datapoint.
        # Actual sampling of multiple datapoints and combining them is handled
        # in a dedicated dataset wrapper (see ExampleDatasetMosaicImageDataset).
        # When used alone, it is a no-op.
        return datapoint
