...
class Sam3Processor:
    ...
    def __init__(
        ...
        # detection score threshold (per-instance)
        self.confidence_threshold = float(confidence_threshold)
        # pixel threshold for masks; default跟随confidence_threshold以保持兼容
        self.mask_threshold = (
    ...
    def set_confidence_threshold(self, threshold: float, state=None):
        """
        Sets the confidence threshold for detections (boxes/masks as instances).

        注意：这是“实例级”的分数阈值，决定哪些预测会被保留；
        像素级的掩码二值阈值由 self.mask_threshold 控制。
        """
    ...
    @torch.inference_mode()
    def set_mask_threshold(self, threshold: float, state=None):
        """
        Sets the per-pixel threshold for masks (用于 state['masks'] 二值化).
        """
        self.mask_threshold = float(threshold)
        if state is not None and "masks_logits" in state:
            masks_prob = state["masks_logits"]
            state["masks"] = masks_prob > self.mask_threshold
        return state
