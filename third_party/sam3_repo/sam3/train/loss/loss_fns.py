...
class IABCEMdetr(LossWithWeights):
    def __init__(
        ...
        # NOTE: This is compatibility workaround to have the same CE loss scale across datasets where the model might predict different number of object queries for different tasks.
...
class Boxes(LossWithWeights):
    def __init__(
        ...
        compute_aux=True,
        apply_loss_to_det_queries_in_video_grounding=True,
        # Optional: give small boxes higher/lower weight via area-aware scaling.
        # areas are computed in normalized cxcywh space (w*h in [0,1]).
        small_box_area_thresh: float = 0.0,
        small_box_weight: float = 1.0,
        large_box_weight: float = 1.0,
    ):
        super().__init__(weight_dict, compute_aux)
        self.apply_loss_to_det_queries_in_video_grounding = (
            apply_loss_to_det_queries_in_video_grounding
        ...
        # area-aware weighting; default设置为无效（阈值=0 或权重=1）
        self.small_box_area_thresh = float(small_box_area_thresh)
        self.small_box_weight = float(small_box_weight)
        self.large_box_weight = float(large_box_weight)
    ...
    def get_loss(self, outputs, targets, indices, num_boxes):
        ...
        # 可选：根据 GT box 面积对 loss 做加权，小目标权重大一些。
        if self.small_box_area_thresh > 0.0 and (
            self.small_box_weight != 1.0 or self.large_box_weight != 1.0
        ):
            # target_boxes: (N,4) in cxcywh, normalized
            with torch.no_grad():
                w = target_boxes[..., 2].clamp(min=0.0)
                h = target_boxes[..., 3].clamp(min=0.0)
                area = w * h  # (N,)
                weights = torch.where(
                    area < self.small_box_area_thresh,
                    torch.full_like(area, self.small_box_weight),
                    torch.full_like(area, self.large_box_weight),
                )
            # broadcast到 bbox / giou loss 上
