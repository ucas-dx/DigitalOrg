...
# ConvNeXt V1（学生）
HAS_CONVNEXT_V1 = False
try:
    from ultralytics.nn.modules.convnextv1 import ConvNeXtV1  # type: ignore
    HAS_CONVNEXT_V1 = True
except Exception:
    try:
        from .convnextv1 import ConvNeXtV1  # local fallback
        HAS_CONVNEXT_V1 = True
    except Exception:
        ConvNeXtV1 = None  # type: ignore
        HAS_CONVNEXT_V1 = False
# ---------------- Paths & defaults ----------------
...
DEFAULT_RESUME_CHECKPOINT = None
...
class ROIRefineNet(nn.Module):
    """ConvNeXt V1 student refine head (ROIRefineNetV1)."""
    ...
    def __init__(
        ...
        super().__init__()
        if not HAS_CONVNEXT_V1:
            raise ImportError("ConvNeXtV1 is unavailable. Please install ultralytics or provide the module.")
        ...
        self.backbone = ConvNeXtV1(
            variant=variant,
            in_chans=3,
            ckpt=convnext_ckpt if convnext_ckpt else None,
            out_indices=(0, 1, 2, 3),
            drop_path_rate=0.0,
            layer_scale=1e-6,
        )
        ...
        self.prompt_encoder = (
            PromptMaskEncoder(out_channels=c0) if (self.build_prompt or self.use_prompt) else None
        )
    ...
    def forward(
        self,
        roi_img_bchw: torch.Tensor,
        prompt_mask_bchw_256: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        return _interp(logits64, size=(256, 256), mode="bilinear")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--convnext-variant', type=str, default='atto')
    parser.add_argument('--convnext-ckpt', type=str, default='')
    # 是否采用 SAM 掩码作为提示
...
def main() -> None:
    ...
    def _safe_load_refine_state(target: nn.Module, state: Dict[str, torch.Tensor], tag: str) -> None:
        tgt = target.state_dict()
        ok: Dict[str, torch.Tensor] = {}
        skip = []
        for k, v in state.items():
            if k in tgt and tuple(tgt[k].shape) == tuple(getattr(v,'shape',())):
                ok[k] = v
            else:
                skip.append((k, tuple(getattr(v, 'shape', [])), tuple(tgt[k].shape if k in tgt else ())))
        missing, unexpected = target.load_state_dict(ok, strict=False)
        _maybe_print(f"[resume:{tag}] loaded={len(ok)} skip={len(skip)} missing={len(missing)} unexpected={len(unexpected)}")
        if skip:
            k, ck, tk = skip[0]
            _maybe_print(f"[resume:{tag}] example mismatch: {k} ckpt={ck} tgt={tk}")
    ...
    if args.resume:
        ...
        if resume_path.exists():
            ...
            if 'refine' in ckpt:
                _safe_load_refine_state(refine_net, ckpt['refine'], tag=str(resume_path))
